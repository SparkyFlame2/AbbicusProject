...
def simulate_battle():
    ...
    if choice == "1" and sentinels > 0:
        ...
        if use == "a" and advanced_oso >= sentinels:
            ...
            battle_summary(0, sentinels, 0, sentinels)
            sentinels = 0
        ...
        if use == "a" and advanced_oso * 5 >= enforce.enforcers:
            ...
            enforce.enforcers = 0
            battle_summary(enforce.enforcers, 0, 0, lost)
        elif use == "b" and enforce.oso_units >= (enforce.enforcers + 1) // 2:
            enforcers_defeated = enforce.enforcers
            lost = (enforcers_defeated + 1) // 2
            enforce.oso_units -= lost
            award_victory_points(enforcers_defeated)
            enforce.enforcers = 0
            battle_summary(enforcers_defeated, 0, lost, 0)
...
def battle_summary(enforcers_defeated, sentinels_defeated, oso_lost, adv_oso_lost):
    print("\n[BATTLE SUMMARY]")
    print(f"You defeated {enforcers_defeated} Enforcers and {sentinels_defeated} Sentinels.")
    print(f"You lost {oso_lost} Oso and {adv_oso_lost} Advanced Oso in the battle.")
